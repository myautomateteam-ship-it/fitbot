...
def build_context(profile: dict, today: dict, patterns: dict, notes: list) -> str:
    ...
    context = f"""ПРОФИЛЬ (факты из базы):
...
def build_system_prompt(context: str, style: str = 'friendly') -> str:
    styles = {
        'friendly': 'дружелюбно и тепло',
        'strict': 'строго и по делу',
        'humor': 'с юмором и легко'
    }
...
async def extract_data(message: str) -> dict:
    """Извлекаем структурированные данные из любого сообщения юзера"""
    prompt = f"""Проанализируй сообщение. Только JSON, без текста вокруг.

Сообщение: "{message}"

{{
  "health_update": "травма/боль или null",
  "weight_update": число или null,
  "mood": "настроение или null",
  "energy": число 1-5 или null,
  "sleep_hours": число или null,
  "sleep_intent": true если идёт спать иначе false,
  "workout_done": true/false/null,
  "intent": "что хочет или null",
  "preference": "личное предпочтение или null",
  "custom_note": "важная инфо или null"
}}"""
...
async def chat(user_id: int, message: str, profile: dict = None,
               today: dict = None, patterns: dict = None, notes: list = None) -> str:
...
async def calculate_nutrition(food: str) -> dict | None:
...
{{"calories":число,"protein":число,"fat":число,"carbs":число,"description":"что посчитал","is_approximate":true/false}}"""
    try:
        r = await client.chat.completions.create(
            model=MODEL,
            messages=[{"role":"system","content":"Диетолог. Точные КБЖУ. Только JSON."},
...
- Травмы (ИСКЛЮЧИТЬ упражнения): {injuries}
...
💡 Совет: [1 предложение]"""
    ...
    r = await client.chat.completions.create(
        model=MODEL,
        messages=[{"role":"system","content":"Тренер. Учитывай травмы — это критично."},
                  {"role":"user","content":prompt}],
...
async def process_onboarding(step: str, answer: str) -> dict:
    steps_config = {
        ...
        "ask_target":     ("target_weight", "число 30-200"),
...
async def generate_reminder_text(rtype: str, profile: dict, today: dict, patterns: dict) -> str:
    ...
    r = await client.chat.completions.create(
        model=MODEL,
        messages=[{"role":"system","content":f"Тренер Макс. {context}"},
                  {"role":"user","content":prompts.get(rtype,"Короткое мотивирующее сообщение.")}],
        max_tokens=120, temperature=0.8
    )
    return r.choices[0].message.content


def calculate_bmr_tdee(age, gender, height, weight, activity_level, goal) -> dict:
    if gender == "male":
